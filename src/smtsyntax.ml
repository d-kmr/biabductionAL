...
module Logic = struct
  ...
  let pp fmt logic =
    match logic with
    | AUFLIA -> Format.fprintf fmt "AUFLIA"
    | AUFLIRA -> Format.fprintf fmt "AUFLIRA"
    | AUFNIRA -> Format.fprintf fmt "AUFNIRA"
    | LIA -> Format.fprintf fmt "LIA"
    | LRA -> Format.fprintf fmt "LRA"
    | QF_ABV -> Format.fprintf fmt "QF_ABV"
    | QF_AUFBV -> Format.fprintf fmt "QF_AUFBV"
    | QF_AUFLIA -> Format.fprintf fmt "QF_AUFLIA"
    | QF_AX -> Format.fprintf fmt "QF_AX"
    | QF_BV -> Format.fprintf fmt "QF_BV"
    | QF_IDL -> Format.fprintf fmt "QF_IDL"
    | QF_LIA -> Format.fprintf fmt "QF_LIA"
    | QF_LRA -> Format.fprintf fmt "QF_LRA"
    | QF_NIA -> Format.fprintf fmt "QF_NIA"
    | QF_NRA -> Format.fprintf fmt "QF_NRA"
    | QF_RDL -> Format.fprintf fmt "QF_RDL"
    | QF_UF -> Format.fprintf fmt "QF_UF"
    | QF_UFBV -> Format.fprintf fmt "QF_UFBV"
    | QF_UFIDL -> Format.fprintf fmt "QF_UFIDL"
    | QF_UFLIA -> Format.fprintf fmt "QF_UFLIA"
    | QF_UFLRA -> Format.fprintf fmt "QF_UFLRA"
    | QF_UFNRA -> Format.fprintf fmt "QF_UFNRA"
    | UFLRA -> Format.fprintf fmt "UFLRA"
    | UFNIA -> Format.fprintf fmt "UFNIA"
...
module Sort = struct
  ...
  let rec pp fmt s =
    ...
    | App(hd,sorts) -> Format.fprintf fmt "(%s %a)" hd (pp_list "" " " pp) sorts
...
module Keyword = struct
  ...
  let pp fmt kw =
    match kw with
    | K0 kwd -> Format.fprintf fmt ":%s" kwd
    | K1 kwd -> Format.fprintf fmt "%s" kwd

  let pps _ kw =
    match kw with
    | K0 kwd -> Format.sprintf ":%s" kwd
    | K1 kwd -> Format.sprintf "%s" kwd
 ...
 *)
  ...
  let pps_domain (_:unit) dom =
    match dom with
    | Nat -> Format.sprintf "nat"
    | Int -> Format.sprintf "int"

  let string_of_domain dom = pps_domain () dom
  ...
  let rec pp_smt2 fmt exp =
    ...
    | App(func,args) -> Format.fprintf fmt "(%s %a)" func (pp_list "" " " pp_smt2) args
    | All(dom,vvv,e) -> Format.fprintf fmt "(forall-%a(%a) %a)" pp_domain dom (Strset.pp ",") vvv pp_smt2 e
    | Ex(dom,vvv,e) -> Format.fprintf fmt "(exists-%a(%a) %a)" pp_domain dom (Strset.pp ",") vvv pp_smt2 e
    | Let(binds,e) ->
       let pp_bind fmt (v,e) = Format.fprintf fmt "(%s,%a)" v pp_smt2 e in
       Format.fprintf fmt "(let %a %a)" (pp_list "" " " pp_bind) binds pp_smt2 e
    | Att(e,attrs) -> Format.fprintf fmt "(Attr %a %a)" pp_smt2 e (pp_list "" " " pp_attr) attrs
  and pp_attr fmt (kw,exps) = Format.fprintf fmt "%a %a" Keyword.pp kw (pp_list "" " " pp_smt2) exps

  let rec pps_smt2 fmt exp =
    match exp with
    | Var v -> Format.sprintf "%s" v
    | Int n -> Format.sprintf "%d" n
    | PosInf -> Format.sprintf "PosInf"
    | NegInf -> Format.sprintf "NegInf"
    | App(func,args) -> Format.sprintf "(%s %a)" func (pps_list "" " " pps_smt2) args
    | All(dom,vvv,e) -> Format.sprintf "(forall-%a(%a) %a)" pps_domain dom (Strset.pps ",") vvv pps_smt2 e
    | Ex(dom,vvv,e) -> Format.sprintf "(exists-%a(%a) %a)" pps_domain dom (Strset.pps ",") vvv pps_smt2 e
    | Let(binds,e) ->
       let pps_bind _ (v,e) = Format.sprintf "(%s,%a)" v pps_smt2 e in
       Format.sprintf "(let %a %a)" (pps_list "" " " pps_bind) binds pps_smt2 e
    | Att(e,attrs) -> Format.sprintf "(Attr %a %a)" pps_smt2 e (pps_list "" " " pps_attr) attrs
  and pps_attr _ (kw,exps) = Format.sprintf "%a %a" Keyword.pps kw (pps_list "" " " pps_smt2) exps
  ...
  let rec pp fmt exp =
    ...
    | App(func,args) -> Format.fprintf fmt "(%s %a)" func (pp_list "" " " pp) args
    | All(dom,vvv,e) -> Format.fprintf fmt "∀%a:%a(%a)" (Strset.pp ",") vvv pp_domain dom pp e
    | Ex(dom,vvv,e) -> Format.fprintf fmt "∃%a:%a(%a)" (Strset.pp ",") vvv pp_domain dom pp e
    | Let(binds,e) ->
       let pp_bind fmt (v,e) = Format.fprintf fmt "(%s,%a)" v pp e in
       Format.fprintf fmt "Let\n%a\nin\n%a" (pp_list "" " " pp_bind) binds pp e
    | Att(e,attrs) -> Format.fprintf fmt "Attr(%a,%a)" pp e (pp_list "" " " pp_attr) attrs
  and pp_attr fmt (kw,exps) = Format.fprintf fmt "%a %a" Keyword.pp kw (pp_list "" " " pp) exps
  ...
  let rec pps fmt exp =
    ...
    | App(func,args) -> Format.sprintf "(%s %a)" func (pps_list "" " " pps) args
    | All(dom,vvv,e) -> Format.sprintf "∀%a:%a(%a)" (Strset.pps ",") vvv pps_domain dom pps e
    | Ex(dom,vvv,e) -> Format.sprintf "∃%a:%a(%a)" (Strset.pps ",") vvv pps_domain dom pps e
    | Let(binds,e) ->
       let pps_bind fmt (v,e) = Format.sprintf "%s=%a" v pps e in
       Format.sprintf "Let\n%a\nin\n%a" (pps_list "" " " pps_bind) binds pps e
    | Att(e,attrs) -> Format.sprintf "Attr(%a,%a)" pps e (pps_list "" " " pps_attr) attrs
  and pps_attr fmt (kw,exps) = Format.sprintf "%a %a" Keyword.pps kw (pps_list "" " " pps) exps
                           
  let print = Format.printf "@[%a@]" pp

  let println = Format.printf "@[%a@." pp
              
  let to_string exp = pps () exp
...
module Command = struct
  ...
  let rec pp fmt com =
    match com with
    | DecVar(sym,sort)  -> Format.fprintf fmt "(declare-var %s %a)" sym Sort.pp sort
    | DecCons(sym,sort) -> Format.fprintf fmt "(declare-const %s %a)" sym Sort.pp sort
    | DecFn(sym,sorts,sort) -> Format.fprintf fmt "(declare-fun %s (%a) %a)" sym (pp_list "" " " Sort.pp) sorts Sort.pp sort
    | DecFns fdecs ->
       let pp_fdec fmt (f,sorts) = Format.fprintf fmt "(%s %a)" f (pp_list "" " " Sort.pp) sorts in
       Format.fprintf fmt "(declare-funs (%a))" (pp_list "" " " pp_fdec) fdecs
    | DecSt(sym,n) -> Format.fprintf fmt "(declare-sort %s %d)" sym n
    | DefSt(sym,syms,sort) -> Format.fprintf fmt "(define-sort %s (%a) %a)" sym (pp_list "" " " Format.pp_print_string) syms Sort.pp sort
    | DefFn(sym,args,sort,exp) ->
       let pp_arg fmt (x,s) = Format.fprintf fmt "(%s %a)" x Sort.pp s in
       Format.fprintf fmt "(define-fun %s (%a) %a %a)" sym (pp_list "" " " pp_arg) args Sort.pp sort Exp.pp exp
    | DecPreds pdecs ->
       let pp_pdec fmt (p,sorts) =
         if sorts = [] then Format.fprintf fmt "(%s)" p
         else Format.fprintf fmt "(%s %a)" p (pp_list "" " " Sort.pp) sorts
       in
       Format.fprintf fmt "(declare-preds (%a))" (pp_list "" " " pp_pdec) pdecs
    | Ass exp -> Format.fprintf fmt "(assert %a)" Exp.pp exp
    | GetAss -> Format.fprintf fmt "(get-assertions)"
    | CheckSat -> Format.fprintf fmt "(check-sat)"
    | GetPrf -> Format.fprintf fmt "(get-proof)"
    | GetUnsat -> Format.fprintf fmt "(get-unsat-core)"
    | GetVal exps -> Format.fprintf fmt "(get-value %a)" (pp_list "" " " Exp.pp) exps
    | GetAsgn -> Format.fprintf fmt "(get-assignment)"
    | GetOpt kw -> Format.fprintf fmt "(get-option %a)" Keyword.pp kw
    | SetOpt(kw,exp) -> Format.fprintf fmt "(set-option %a %a)" Keyword.pp kw Exp.pp exp
    | GetInfo kw -> Format.fprintf fmt "(get-info %a)" Keyword.pp kw
    | SetInfo(kw,exp) -> Format.fprintf fmt "(set-info %a %a)" Keyword.pp kw Exp.pp exp
    | Exit -> Format.fprintf fmt "(exit)"
    | Push None -> Format.fprintf fmt "(push)"
    | Push (Some n) -> Format.fprintf fmt "(push %d)" n
    | Pop None -> Format.fprintf fmt "(pop)"
    | Pop (Some n) -> Format.fprintf fmt "(pop %d)" n
    | Echo str -> Format.fprintf fmt "(echo %s)" str
    | GetModel -> Format.fprintf fmt "(get-model)"
    | Reset -> Format.fprintf fmt "(reset)"
    | Display exp -> Format.fprintf fmt "(display %a)" Exp.pp exp
    | Eval(exp,[]) -> Format.fprintf fmt "(eval %a)" Exp.pp exp
    | Eval(exp,kwexps) ->
       let pp_kwexp fmt (kw,e) = Format.fprintf fmt "(%a %a)" Keyword.pp kw Exp.pp e in
       Format.fprintf fmt "(eval %a %a)" Exp.pp exp (pp_list "" " " pp_kwexp) kwexps
    | Model -> Format.fprintf fmt "(model)"
    | Simpfy(exp,[]) -> Format.fprintf fmt "(simplify %a)" Exp.pp exp
    | Simpfy(exp,attrs) -> Format.fprintf fmt "(simplify %a %a)" Exp.pp exp (pp_list "" " " Exp.pp_attr) attrs
    | DecDT(syms, dtypes) ->
       Format.fprintf fmt "(declare-datatypes (%a) (%a))" (pp_list "" " " Format.pp_print_string) syms (pp_list "" " " pp_datatype) dtypes
  and pp_datatype fmt (sym,cdecls) =
    match cdecls with
    | [] -> Format.fprintf fmt "%s" sym
    | _  -> Format.fprintf fmt "(%s %a)" sym (pp_list "" " " pp_consdecl) cdecls
  and pp_consdecl fmt (sym,adecls) =
    match adecls with
    | [] -> Format.fprintf fmt "%s" sym
    | _  -> Format.fprintf fmt "(%s %a)" sym (pp_list "" " " pp_accdecl) adecls
  and pp_accdecl fmt (sym,sort) = Format.fprintf fmt "(%s %a)" sym Sort.pp sort
...
module SMTprog = struct
  ...
  let pp fmt (prog : t) =
    ...
    Format.fprintf fmt "(set-logic %a)\n\n" Logic.pp logic;
