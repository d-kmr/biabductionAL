...
let dbgAT tag fmt =
  if tag |<- !p_opt then
    ANSITerminal.printf fmt
  else
    (fun _ _ -> ())
;;

let dbg_color color tag =
  Printf.ksprintf (fun s -> if tag |<- !p_opt then dbgAT tag [Bold; ANSITerminal.Foreground color] "%s\n" s else ())
;;
let dbg_bl tag = dbg_color ANSITerminal.Blue tag;;
let dbg_rd tag = dbg_color ANSITerminal.Red tag;;
let dbg_gr tag = dbg_color ANSITerminal.Green tag;;
let dbg_yl tag = dbg_color ANSITerminal.Yellow tag;;
...
let new_prog_var _ =
  let t = !cfunc ^ "#_" ^ (string_of_int !num) in
  num.contents <- (!num + 1); t;;
