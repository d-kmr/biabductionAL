...
module Attr = struct
  ...
  let pp fmt a =
    ...
    | ARRAY nn -> () (* Format.fprintf fmt "@ARRAY[%a]" (pp_list "" "," Format.pp_print_int) nn *)
    ...
    | NESTED -> Format.fprintf fmt "@NESTED"
    | QUESTION -> Format.fprintf fmt "@QUESTION"
    | DOTDOT -> Format.fprintf fmt "@DOTDOT"
  ...
  let pps (_:unit) a =
    ...
    | STRUCT sname -> Format.sprintf "@STRUCT %s" sname
    | EXQ -> "" (* Format.sprintf "@EXQ" *)
    | ARRAY nn -> "" (* Format.sprintf "@ARRAY[%a]" (pps_list "" "," pps_int) nn *)
    ...
    | PROTO -> Format.sprintf "@PROTO"
    | FUNC -> Format.sprintf "@FUNC"
    ...
    | ACUTE -> Format.sprintf "@AQUTE"
    | SIMPLE n -> "" (* Format.sprintf "@SIMPLE(%d)" n *)
    | INDIRECT -> Format.sprintf "@INDIRECT"
    | TEMP -> Format.sprintf "@TEMP"
    | PHANTOM -> Format.sprintf "@PHANTOM"
    | OFFSET _ -> Format.sprintf "@OFFSET"
               
  let to_string a = pps () a
...
module Attrs = struct
  ...
  let pp fmt aa = pp_list "" "" Attr.pp fmt (elements aa)

  let pps _ aa = pps_list "" "" Attr.pps () (elements aa)
    
  let to_string aa = pps () aa
...
module SHterm = struct
  ...
  let rec pp fmt t =
    ...
    | Fcall(g,tt) -> Format.fprintf fmt "%s(%a)" g (pp_list "" "," pp) tt
  and pp1 fmt t =
    match t with
  ...
  let rec pps fmt t =
    match t with
    | Var(v,attrs) when attrs = Attrs.empty -> Format.sprintf "%s" v
    | Var(v,attrs) -> Format.sprintf "%s%@%a" v Attrs.pps attrs
    | Int n -> if n >= 0 then Format.sprintf "%d" n else Format.sprintf "(%d)" n
    | PosInf -> Format.sprintf "PosInf"
    | NegInf -> Format.sprintf "NegInf"
    | Add tt -> Format.sprintf "%a" (pps_list "ERROR" "+" pps1) tt
    | Sub tt -> Format.sprintf "%a" (pps_list "ERROR" "-" pps1) tt
    | Mul(t1,t2) -> Format.sprintf "%a*%a" pps1 t1 pps1 t2
    | Div(t1,t2) -> Format.sprintf "%a/%a" pps1 t1 pps1 t2
    | Mod(t1,t2) -> Format.sprintf "%a %% %a" pps1 t1 pps1 t2
    | Shr(t1,t2) -> Format.sprintf "%a>>%a" pps1 t1 pps1 t2
    | Shl(t1,t2) -> Format.sprintf "%a<<%a" pps1 t1 pps1 t2
    | Band(t1,t2) -> Format.sprintf "%a band %a" pps1 t1 pps1 t2
    | Bor(t1,t2) -> Format.sprintf "%a bor %a" pps1 t1 pps1 t2
    | Bxor(t1,t2) -> Format.sprintf "%a bxor %a" pps1 t1 pps1 t2
    | Bnot t1 -> Format.sprintf "~%a" pps1 t1
    | Fcall(g,tt) -> Format.sprintf "%s(%a)" g (pps_list "" "," pps) tt
  and pps1 fmt t =
    match t with
    | Var _ | Int _ | Mul(_,_) | Mod(_,_) -> Format.sprintf "%a" pps t
    | _ -> Format.sprintf "(%a)" pps t
  ...
  let rec pp_data fmt t =
    ...
    | Fcall(g,tt) -> Format.fprintf fmt "Fcall(%s,%a)" g (pp_list "" "," pp_data) tt
         
  let to_string t = pps () t
  ...
  let getTypes t aa =
    ...
    | true,_,_ -> raise (Error (Format.sprintf "%a has an illegal type: ARRAY + PTR/STRUCT" pps t))
...
module Tset = struct
  ...
  let varProfile t =
    ...
    | 0 ->
       if is_empty (getVarsByAttrs (Attrs.of_list [BAR;DOT;DOTDOT]) t)
       then ("",0)
       else ("",1)
    | 1 ->
  ...
  let pp fmt x = pp_list "" "," T.pp fmt (elements x)
...
module SHpure = struct
  ...
  let rec pp fmt p =
    ...
    | All(dom,vvv,q) -> Format.fprintf fmt "ALL%a[%a](%a)"
                          pp_domain dom
                          (pp_list "" "," Format.pp_print_string) (Strset.elements vvv)
                          pp q
...
(* 's' is used for them	*)
  ...
  let rec pp fmt (s: t) =
    ...
    | Ind(p,tt) ->
       Format.fprintf fmt "%s(%a)" p (pp_list "" "," T.pp) tt
  and pp_cc fmt cc =
    let pp_f_t fmt (f,t) = Format.fprintf fmt "%s:%a" f T.pp t in
    Format.fprintf fmt "%a" (pp_list "" "," pp_f_t) cc
...
(* 'h' is used for them			*)
  ...
  let pp fmt (h : t) =
    ...
    let ex = if Strset.is_empty vvv then "" else "Ex " in
    let dot = if Strset.is_empty vvv then "" else ". " in
    Format.fprintf fmt "%s%a%s%a & %a" ex (pp_list "" "," Format.pp_print_string) vv dot P.pp p SS.pp ss
...
module Segments = struct
  ...
  let pp fmt = Format.fprintf fmt "[%a]" (pp_list "" "," Invl.pp)
...
module MIfunctionIO = struct
  ...
  let pp_funproto fmt (func: fproto list) =
    ...
    | (rettypes,fname,params)::_ ->
       ...
       Format.fprintf fmt "%a %s (%a)" Attrs.pp rettypes fname (pp_list "" ", " pp_param1) params
    
  let pp_store fmt (s : store) =
    let pp1 fmt (v,t) = Format.fprintf fmt "%s=%a" v T.pp t in
    Format.fprintf fmt "[%a]" (pp_list "" ", " pp1) s
