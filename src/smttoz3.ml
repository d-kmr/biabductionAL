...
module Mode = struct
  ...
  let pp fmt mode =
    match mode with
    | Arithmetic str -> Format.fprintf fmt "@[Arithmetic:%s@]" str
    | BitVector -> Format.fprintf fmt "@[BitVector@]"

  let println mode = Format.fprintf stdout "@[%a@." pp mode    
...
module SimpleExp = struct
  ...
  let pp fmt (se : t) =
    match se with
    | Cons str -> Format.fprintf fmt "%s" str
    | Int n -> Format.fprintf fmt "%d" n
    | OTHER -> Format.fprintf fmt "OTHER"

  let pps fmt (se : t) =
    match se with
    | Cons str -> Format.sprintf "%s" str
    | Int n -> Format.sprintf "%d" n
    | OTHER -> Format.sprintf "OTHER"
  ...
  let exp2int e =
    ...
    with
      ...
      with
        _ -> (* real fail *)
        Format.printf "@[SimpleExp: Non-numeral %s@." eStr';
        failwith ""
  ...
  let to_int ctx e =
    ...
    | _,true -> exp2int e
    | _,_ -> Format.printf "@[SimpleExp.to_int: Non-numeral %s@." (ZE.to_string e); failwith ""
...
module SatcheckResult = struct
  ...
  let pp_model fmt model =
    let pp1 fmt (x,n) = Format.fprintf fmt "(%s,%d)" x n in
    Format.fprintf fmt "%a" (pp_list "" " " pp1) model

  let pp_unsatcore fmt uc =
    Format.fprintf fmt "%s" (List.fold_left (fun s e -> s ^ (Z3.Expr.to_string e)) "" uc)
    
  let println res =
    match res with
    | Model mods -> Format.printf "@[%a\n@." pp_model mods
...
let checkCommands mode modelflag ucflag vvInt vvNat commands : SatcheckResult.t =
  ...
  in
  Ftools.pf_s "SATdebug" print_endline "------------------------------";  
  Ftools.pf_s "SATdebug" print_endline "Input for checkCommands";
  Ftools.pf_s "SATdebug" (List.iter C.println) commands;
  Ftools.pf_s "SATdebug" print_endline "------------------------------";    
  let (ctx,sdata,fdata) = ectx in
  ...
  let rec mk_mysolver solver' com =
	...
	(* ignored since they are out of scope *)
      ...
      | C.GetVal exp -> mk_mysolver solver' tlcom (* dummy *)
  in
  ...
  Ftools.pf_s "Z3"
    (fun _ -> Printf.printf "[Z3 Solver Status]\nTimeout: %s\nCommands:\n%s" timeoutStr (Solver.to_string solver)) ();

  match Solver.check solver [],modelflag,ucflag with
  | Z3.Solver.SATISFIABLE,true,_ ->
     Ftools.pf_s "Z3" (fun _ -> Printf.printf "Result: SAT\n\n") ();
     begin
  ...
  | Z3.Solver.UNSATISFIABLE,_,true ->
     Ftools.pf_s "Z3" (fun _ -> Printf.printf "Result: UNSAT\n\n") ();     
  ...
  | Z3.Solver.UNKNOWN,_,_ ->
     Ftools.pf_s "Z3" (fun _ -> Printf.printf "Result: UNKNOWN\n\n") ();
     (*
     Format.printf "@[Command: %a@." (pp_list "" "\n" Command.pp) commands;
      *)
     Format.printf "@[checkCommands: solver returns UNKNOWN@.";
     raise UNKNOWN
;;
...
let checkCommandsSwitch mode modelflag ucflag vvInt vvNat commands =
  ...
  | UnsatCore _ when ucflag ->
     Ftools.pf_s "Z3" print_endline "\nSecond Execution of Z3 for making UNSAT-CORE";
...
(*  let posInf_eq_maxInt = posInf =^= (Exp.Int max_int) in
  ...
  let cUbIntVars = List.map (fun v -> C.Ass (Exp.App("<",[Exp.Var v;posInf]))) vvInt in
  let cLbIntVars = List.map (fun v -> C.Ass (Exp.App("<",[negInf;Exp.Var v]))) vvInt in
  let cUbNatVars = List.map (fun v -> C.Ass (Exp.App("<",[Exp.Var v;posInf]))) vvNat in  
  let cLbNatVars = List.map (fun v -> C.Ass (Exp.App("<=",[Exp.Int 0;Exp.Var v]))) vvNat in
  let commandsArith = 
    let cPosInf = [C.DecVar(_PosInf,myInt);C.Ass posInf_eq_maxInt] in
    let cNegInf = [C.DecVar(_NegInf,myInt);C.Ass negInf_eq_minInt] in
    let cDeclVars = List.map (fun v -> C.DecVar (v,myInt)) (vvInt@vvNat) in
    cPosInf @ cNegInf @ cDeclVars @ cUbIntVars @ cLbIntVars @ cUbNatVars @ cLbNatVars @ commands
  in
  let commandsBV = 
    let cPosInf = [C.DecVar(_PosInf,myBV32);C.Ass posInf_eq_maxInt] in
    let cNegInf = [C.DecVar(_NegInf,myBV32);C.Ass negInf_eq_minInt] in
    let cDeclVars = List.map (fun v -> C.DecVar (v,myBV32)) (vvInt@vvNat) in
    cPosInf @ cNegInf @ cDeclVars @ cUbIntVars @ cLbIntVars @ cUbNatVars @ cLbNatVars @ commands
  in
  try
  ...
  | UNKNOWN ->
     begin
       Ftools.pf_s "Z3" print_endline "UNKNOWN --> switch to BitVector BV[32] \n";
       try
         checkCommandsSwitch BitVector modelflag ucflag vvInt vvNat commandsBV
       with
       | UNKNOWN ->
          begin
            Ftools.pf_s "Z3" print_endline "UNKNOWN --> switch to Arithmetic LIA \n";            
            try
...
let checkExpIntNat modelflag ucflag ee : SatcheckResult.t =
  ...
  let commands = cAssExpL @ [C.CheckSat] in
  try
...
let checkSatExp modelflag ucflag exp =
  Ftools.pf_s "SATdebug" print_endline "------------------------------";  
  Ftools.pf_s "SATdebug" print_endline "Input for checkSatExp";
  Ftools.pf_s "SATdebug" Exp.println exp;
  Ftools.pf_s "SATdebug" print_endline "------------------------------";
  let res =
    try
...
let checkSatExpL modelflag ucflag ee =
  Ftools.pf_s "SATdebug" print_endline "------------------------------";  
  Ftools.pf_s "SATdebug" print_endline "Input for checkSatExpL";
  Ftools.pf_s "SATdebug" (List.iter Exp.println) ee;
  Ftools.pf_s "SATdebug" print_endline "------------------------------";
  let res =
    try
