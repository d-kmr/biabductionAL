...
let pp_opt none pp fmt opt =
  ...
  | Some u -> Format.fprintf fmt "%a" pp u
;;
let pp_list base sep pp fmt xx =
  match xx with
  | [] -> Format.fprintf fmt "%s" base
  | _ -> Format.pp_print_list ~pp_sep:(fun _ _ -> Format.fprintf fmt sep) pp fmt xx
;;
let pp_string = Format.pp_print_string 
;;
let pp_int = Format.pp_print_int
;;
let pps_list base sep pps (_:unit) xx = (* _:unit is to keep the input format to be the same as pp_list *)
  match xx with
  | [] -> Format.sprintf "%s" base
  | _ -> List.fold_left
           (fun s x ->
             let str = Format.sprintf "%a" pps x in
             if s = "" then str else s ^ sep ^ str)
           "" xx
;;
let pps_string (_:unit) s = s
;;
let pps_int (_:unit) n = string_of_int n
;;
let fst3 (x,_,_) = x;;
let snd3 (_,y,_) = y;;
let thd3 (_,_,z) = z;;
...
module Strset = struct
  ...
  let pp sep fmt vvv = Format.fprintf fmt "%a" (pp_list "" sep Format.pp_print_string) (elements vvv)

  let pps sep (_:unit) vvv = Format.sprintf "%a" (pps_list "" sep pps_string) (elements vvv)

  let print vvv = Format.printf "@[%a@]" (pp ", ") vvv
...
module List_mes = struct
  ...
  let nth' mes print_func ls n =
    ...
    with
      _ ->
      Format.printf "@[Exception from List_mes.nth'!!@.";
      Format.printf "@[Mes: %s@." mes;
      Format.printf "@[List: @." ;
      List.iter print_func ls;
      Format.printf "@[Nth: %d@." n;
      failwith "List_mes.nth"
